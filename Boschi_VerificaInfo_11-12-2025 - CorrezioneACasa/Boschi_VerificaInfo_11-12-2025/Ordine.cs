...
namespace Boschi_VerificaInfo_11_12_2025
{
    public class Ordine : IComparer<Ordine>
    {
        ...
        {
            string prodottiStr = string.Join(", ", Prodotti.Select(p => $"{p.Key.Nome} (Quantità: {p.Value})"));
            return $"Ordine ID: {Id}, Cliente: {NomeCliente}, Data: {DataOrdine}, Dettagli: {Dettagli}, Urgente: {Urgente}, Prodotti: [{prodottiStr}]";
        }
    }
}
